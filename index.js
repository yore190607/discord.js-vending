    ...
    try {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.startsWith("buy")) {
                ...
                const header = { "Content-Type": "application/json", Authorization: `Bearer ${paypay[0].token}`, "User-Agent": "PaypayApp/3.31.202202181001 CFNetwork/1126 Darwin/19.5.0", "Client-Mode": "NORMAL", "Client-OS-Version": "13.3.0", "Client-Type": "PAYPAYAPP", "Network-Status": "WIFI", "System-Locale": "ja", "Client-Version": "3.50.0", "Is-Emulator": "false", "Device-Name": "iPhone8,1", "Client-UUID": paypay[0].uuidc, Timezone: "Asia/Tokyo", "Client-OS-Type": "IOS", "Device-UUID": paypay[0].uuid }
            ...
            if (interaction.customId.startsWith("pay")) {
                ...
                const header = { "Client-Mode": "NORMAL", "Client-OS-Version": "13.3.0", "Client-Type": "PAYPAYAPP", "Network-Status": "WIFI", "System-Locale": "ja", "Client-Version": "3.50.0", "Is-Emulator": "false", "Device-Name": "iPhone8,1", "Client-UUID": data.uidc, Timezone: "Asia/Tokyo", "Client-OS-Type": "IOS", "Device-UUID": data.uid };
                ...
                if (check[0]?.name) {
                    const set = await func.dbupdate({ tabel: "paypay", set: `pass="${data.ps}",token="${response.data.payload.accessToken}",id="${data.id}",uuid="${data.uid}",uuidc="${data.uidc}"`, v: `name="${data.name}"` });
                    ...
                    const set = await func.dbset({ tabel: "paypay", v: `"${data.name}","${data.ps}","${response.data.payload.accessToken}","${data.id}","${data.uid}","${data.uidc}"` });
                    if (!set) return await interaction.reply({ embeds: [{ title: "エラー", description: `データの保存に失敗しました。`, color: 0x3aeb34 }], ephemeral: true });
                    await interaction.reply({ embeds: [{ title: "お知らせ", description: `設定しました。`, color: 0x3aeb34 }], ephemeral: true });
                };
            };
